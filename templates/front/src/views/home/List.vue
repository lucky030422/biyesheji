...
// 元素可见时，增加动画
let { stop: productStop } = useIntersectionObserver(
  boxRef,
  ([{ isIntersecting }]) => {
    if (isIntersecting) {
...
<template>
  <div class="goods-4" ref="boxRef">
    <div class="inner">
      <div class="biaoti">
        <div class="subtitle">{{ data.subtitle }}</div>
        <div class="title">{{ data.comments }}</div>
        <i></i>
        <el-button class="more" color="#848C74" @click="moreEvent(data.tableName)">
          更多>>
        </el-button>
      </div>
...
.goods-4 .content .item .img-box .cover {
  width: 100%;
  height: 280px;
  object-fit: cover;
  transition: transform 0.3s linear;
}
.goods-4 .content .item:hover .img-box .cover {
  transform: scale(1.05);
}
.goods-4 .content .item .info-box {
  width: 100%;
  height: auto;
...
.goods-4 .content .item .info-box .text {
  color: #333;
}
.goods-4 .content .item .info-box .price {
  font-size: 16px;
  color: #e72c59;
  font-weight: 600;
  margin-bottom: 6px;
}
.goods-4 .content .item .info-box .user {
  margin: 0 0 6px;
}
.goods-4 .content .item .info-box .time {
  margin: 0 0 6px;
}
.goods-4 .content .item .info-box .other {
  margin: 0 6px 6px 0;
