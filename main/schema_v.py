...
def schemaName_deepseek_askai(request):
    if request.method in ["POST", "GET"]:
        ...
        response = client.chat.completions.create(
            model="deepseek-chat",
            messages=[
                {"role": "user", "content": req_dict['ask']},
            ],
            stream=False
        )
...
def schemaName_updateColumn(request,tableName,type):
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": "成功", "data": {}}
        req_dict = request.session.get("req_dict")
        csuUpdateColumn = req_dict.get("csuUpdateColumn")
        csuUpdateColumnValue = req_dict.get("csuUpdateColumnValue")
        csuConditionColumn = req_dict.get("csuConditionColumn")
        csuConditionColumnValue = req_dict.get("csuConditionColumnValue")
        allModels = apps.get_app_config('main').get_models()
        for m in allModels:
            if m.__tablename__ == tableName:
                try:
                    query_params = {csuConditionColumn: csuConditionColumnValue}
                    update_data = {csuUpdateColumn: csuUpdateColumnValue}
                    find_object = m.objects.filter(**query_params)
                    if type =="3":#删除
                        find_object.delete()
                    elif type =="2":#数字
                        for obj in find_object:
                            find_csuUpdateColumn = getattr(obj, csuUpdateColumn)
                            if csuUpdateColumnValue.startswith('+'):
                                result_csuUpdateColumnValue  = float(csuUpdateColumnValue[1:])+float(find_csuUpdateColumn)
                            elif csuUpdateColumnValue.startswith('-'):
                                result_csuUpdateColumnValue  = float(find_csuUpdateColumn) - float(csuUpdateColumnValue[1:])
                            elif csuUpdateColumnValue.startswith('*'):
                                result_csuUpdateColumnValue  = float(find_csuUpdateColumn) * float(csuUpdateColumnValue[1:])
                            elif csuUpdateColumnValue.startswith('/'):
                                result_csuUpdateColumnValue  = float(float(find_csuUpdateColumn)/float(csuUpdateColumnValue[1:]))
                            else:
                                pass
                            if result_csuUpdateColumnValue<0:
                                msg["code"] = crud_error_code
                                msg["msg"] = "余数不足，更新失败！"
                                return JsonResponse(msg)
                            setattr(obj, csuUpdateColumn, result_csuUpdateColumnValue)
                            obj.save(update_fields=[csuUpdateColumn])
                    else:
                        find_object.update(**update_data)
                except Exception as e:
                    msg["code"] = crud_error_code
                    msg["msg"] = str(e.__str__())
        return JsonResponse(msg)

def schemaName_deleteColumn(request,tableName):
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": "成功", "data": {}}
        req_dict = request.session.get("req_dict")
        csuConditionColumn = req_dict.get("csuConditionColumn")
