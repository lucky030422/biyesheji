...
def yonghu_email_login(request):
    if request.method in ["POST", "GET"]:
        msg = {'code': normal_code, "msg": mes.normal_code}
        req_dict = request.session.get("req_dict")
        emailcode = req_dict["emailcode"]
        ...
        if __sfsh__ == '是':
            if datas[0].get('sfsh') != '是':
                ...
                return JsonResponse(msg, encoder=CustomJsonEncoder)
        # 判断emailcode是否正确
        emailcode_result = emailregistercode.objects.filter(email=email, code=emailcode,role="用户").order_by('-addtime').first()
...
def yonghu_resetPass(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        yonghu.objects.filter(**{username_str: username}).update(**{password_str: init_pwd})
...
def yonghu_accountList(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        try:
            result = [{"id": record.id, "account": record.zhanghao ,"email":record.email  } for record in records]
            msg["data"] = result
        except:
            pass
        return JsonResponse(msg, encoder=CustomJsonEncoder)
...
def yonghu_sendemail_content(request):
    '''
    发送邮箱接口
    '''
    if request.method in ["POST", "GET"]:
...
def yonghu_sendemail_login(request):
    '''
    发送登录验证码
    '''
    if request.method in ["POST", "GET"]:
        req_dict = request.session.get("req_dict")
        email = req_dict['email']
        user =yonghu.objects.filter(email=email).first()
        if user is None:
            return JsonResponse({
                "msg": "邮箱不存在",
        ...
        to = []
        to.append(email)
        send_mail('用户注册', '您的登录验证码是【'+''.join(code)+'】，请不要把验证码泄漏给其他人，如非本人请勿操作。', 'yclw9@qq.com', to, fail_silently = False)
        cursor = connection.cursor()
        cursor.execute("insert into emailregistercode(email,role,code) values('"+email+"','用户','"+''.join(code)+"')")
